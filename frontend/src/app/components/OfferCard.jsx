...
import { CheckCircle, Sparkles, TrendingUp, Shield, ArrowRight } from 'lucide-react';
import { motion } from 'framer-motion';

const OfferCard = ({ title, price, description, benefits, popular = false }) => {
  ...
  const isPremium = title.toLowerCase().includes('premium') || title.toLowerCase().includes('ultimate');
  const isConfort = title.toLowerCase().includes('confort');

  const getPlanIcon = () => {
    if (isPremium) return <Sparkles className="w-8 h-8 text-amber-400" />;
    if (isConfort) return <Shield className="w-8 h-8 text-blue-400" />;
    return <Shield className="w-8 h-8 text-emerald-400" />;
  };

  const getGradient = () => {
    if (isPremium) return "from-slate-800 to-slate-900 border-amber-500/30";
  ...
  return (
    ...
    >
      ...
      <div className="mb-8 text-left relative z-10">
        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 bg-slate-800/50 border border-slate-700 group-hover:scale-110 transition-transform duration-300 ${isPremium ? 'shadow-amber-500/20 shadow-lg' : ''}`}>
          {getPlanIcon()}
        </div>
