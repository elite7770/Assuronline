...
const Claims = () => {
  ...
  const inputClass = "w-full px-4 py-3 bg-slate-800/80 border border-slate-600/60 text-white rounded-xl focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-all duration-200 placeholder-slate-500 hover:border-slate-500";
  ...
  return (
    <div className="min-h-screen bg-slate-900 relative">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_left,_var(--tw-gradient-stops))] from-red-900/15 via-slate-900 to-slate-900 pointer-events-none" />
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-16">
              ...
              {[1, 2, 3, 4].map((step) => (
                <div key={step} className="flex items-center flex-1">
                  <div className="flex flex-col items-center">
                    ...
                    <span className={`mt-2 text-xs font-medium text-center transition-colors duration-300 ${currentStep >= step ? 'text-red-400' : 'text-slate-400'
                      }`}>
                      {step === 1 && 'Incident'}
                      {step === 2 && 'Véhicule'}
                      {step === 3 && 'Dégâts'}
        ...
        <div className="bg-slate-800/60 backdrop-blur-sm rounded-2xl shadow-2xl border border-slate-700/50 overflow-hidden mb-12">
          <div className="bg-gradient-to-r from-red-900/30 via-slate-800 to-slate-800 px-6 py-5 border-b border-slate-700/50">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          {claimTypes.map((type) => (
                            <button
                              ...
                              className={`group relative p-6 border-2 rounded-2xl text-left transition-all duration-300 transform hover:scale-[1.02] hover:shadow-xl ${formData.type === type.value
                            ...
                            >
                              ...
                              {/* Type Icon */}
                              <div className={`p-4 rounded-xl w-fit mb-4 transition-all duration-300 ${formData.type === type.value
                                ? 'bg-gradient-to-r from-red-500 to-red-600 shadow-lg'
                                : 'bg-slate-600 group-hover:bg-red-500'
                                }`}>
                                <type.icon className={`h-8 w-8 transition-all duration-300 ${formData.type === type.value
                                  ? 'text-white'
                              ...
                              <div className="space-y-2">
                                <h3 className={`font-bold text-xl transition-colors ${formData.type === type.value ? 'text-red-300' : 'text-white'
                                  }`}>{type.label}</h3>
                                <p className={`text-sm transition-colors ${formData.type === type.value ? 'text-red-200' : 'text-slate-300'
                                  }`}>{type.description}</p>
                              </div>
                            </button>
                          ))}
                      ...
                      <div className={sectionCardClass}>
                        ...
                        <div className="space-y-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              <label className={labelClass}>Date de l'incident *</label>
                              <input
                                type="date"
                            ...
                            <div>
                              <label className={labelClass}>Heure de l'incident *</label>
                              <input
                                type="time"
                          ...
                          <div>
                            <label className={labelClass}>Lieu de l'incident *</label>
                            <input
                              type="text"
                          ...
                          <div>
                            <label className={labelClass}>Description de l'incident *</label>
                            <textarea
                    ...
                    <div className="space-y-6">
                      <div className={sectionCardClass}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          <div>
                            <label className={labelClass}>Numéro de police *</label>
                            <input type="text" value={formData.policy_id}
                              onChange={(e) => handleInputChange('policy_id', e.target.value)}
                              placeholder="Ex: POL-2024-001234" className={inputClass} required />
                          </div>
                          <div>
                            <label className={labelClass}>Immatriculation *</label>
                            <input type="text" value={formData.license_plate}
                              onChange={(e) => handleInputChange('license_plate', e.target.value)}
                              placeholder="Ex: 12345-A-67" className={inputClass} required />
                          </div>
                          <div>
                            <label className={labelClass}>Marque *</label>
                            <input type="text" value={formData.vehicle_make}
                              onChange={(e) => handleInputChange('vehicle_make', e.target.value)}
                              placeholder="Ex: Toyota, Renault, BMW" className={inputClass} required />
                          </div>
                          <div>
                            <label className={labelClass}>Modèle *</label>
                            <input type="text" value={formData.vehicle_model}
                              onChange={(e) => handleInputChange('vehicle_model', e.target.value)}
                              placeholder="Ex: Corolla, Clio, X3" className={inputClass} required />
                          </div>
                          <div>
                            <label className={labelClass}>Année *</label>
                            <input type="number" value={formData.vehicle_year}
                              onChange={(e) => handleInputChange('vehicle_year', e.target.value)}
                              placeholder="Ex: 2020" min="1990" max="2025"
                    ...
                    <div className="space-y-6">
                      <div className={sectionCardClass}>
                        ...
                        <div className="space-y-5">
                          <div>
                            <label className={labelClass}>Description des dégâts *</label>
                          ...
                          <div>
                            <label className={labelClass}>Estimation des dégâts (MAD)</label>
                            <input type="number" value={formData.amount_estimate}
                              onChange={(e) => handleInputChange('amount_estimate', e.target.value)}
                              placeholder="Montant estimé des réparations" className={inputClass} />
                          </div>
                        </div>
                      </div>
                      ...
                      <div className={sectionCardClass}>
                        ...
                        <div className="space-y-4">
                          <label className="flex items-center gap-3 cursor-pointer group">
                            <div className="relative" onClick={() => handleInputChange('third_party_involved', !formData.third_party_involved)}>
                              <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-all duration-200 ${formData.third_party_involved
                                ? 'bg-red-500 border-red-500'
                                : 'border-slate-500 bg-slate-700 group-hover:border-red-400'
                                }`}>
                                {formData.third_party_involved && <CheckCircle className="h-3 w-3 text-white" />}
                              </div>
                            </div>
                            <span className="text-sm font-medium text-slate-200">Un tiers est impliqué dans cet incident</span>
                          </label>
                          {formData.third_party_involved && (
                            <div>
                              <label className={labelClass}>Informations sur le tiers</label>
                              <textarea value={formData.third_party_details}
                  ...
                  {currentStep === 4 && (
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          <div>
                            ...
                            <input type="tel" value={formData.contact_phone}
                              ...
                              placeholder="Ex: +212 6 12 34 56 78" className={inputClass} required />
                          </div>
                          <div>
                            <label className={labelClass}>Méthode de contact préférée</label>
                            <select value={formData.preferred_contact_method}
                      ...
                      {/* Summary Recap Card */}
                      <div className="bg-gradient-to-br from-red-900/30 to-pink-900/20 border border-red-500/30 rounded-2xl p-6">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                          <div>
                            <div className="text-xs text-slate-400 mb-1">Type de sinistre</div>
                            <div className="text-sm font-bold text-white capitalize">{formData.type || '—'}</div>
                          </div>
                          <div>
                            <div className="text-xs text-slate-400 mb-1">Date</div>
                            <div className="text-sm font-bold text-white">{formData.incident_date || '—'}</div>
                          </div>
                          <div>
                  ...
                  {currentStep === 5 && (
                    <div className="text-center py-12">
                      ...
                      >
                        <h2 className="text-3xl font-bold text-white mb-3">Déclaration soumise !</h2>
                        <p className="text-slate-300 text-base mb-2 max-w-md mx-auto">
                          Votre déclaration de sinistre a été enregistrée avec succès.
                        </p>
                        <p className="text-slate-400 text-sm mb-8">Vous recevrez un email de confirmation avec votre numéro de référence.</p>
                        <div className="flex justify-center gap-4">
                          <button type="button" onClick={() => navigate('/')}
                            className="px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-xl hover:from-blue-700 hover:to-blue-800 transition-all duration-300 font-semibold hover:scale-105">
                            Retour à l'accueil
                          </button>
                          <button type="button" onClick={() => {
                            setCurrentStep(1);
                            setFormData({
                              type: '', incident_date: '', incident_time: '',
                              incident_location: '', description: '', policy_id: '',
                              vehicle_make: '', vehicle_model: '', vehicle_year: '',
                              license_plate: '', damage_description: '', amount_estimate: '',
                              third_party_involved: false, third_party_details: '',
                              contact_phone: '', preferred_contact_method: 'email',
                              emergency_contact: '', documents: []
                            });
                            setSuccess('');
                          }}
                            className="px-6 py-3 bg-slate-700 border border-slate-600 text-white rounded-xl hover:bg-slate-600 transition-all duration-300 font-semibold hover:scale-105">
                ...
                <div className="flex items-center justify-between mt-8 pt-6 border-t border-slate-600/50">
                  <div className="flex items-center space-x-4">
                    ...
                    >
                      <Save className="h-4 w-4 mr-2" />
                      <span className="text-sm font-medium">Sauvegarder</span>
                      {isDraftSaved && (<CheckCircle className="h-4 w-4 ml-2 text-green-400" />)}
                    </button>
                  </div>
                  <div className="flex items-center space-x-4">
                    ...
                    {currentStep < 4 ? (
                      ...
                      >
                        {loading ? (
                          <><div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>Soumission...</>
                        ) : (
                          <>Soumettre la déclaration<CheckCircle className="h-4 w-4 ml-2" /></>
                        )}
                      </button>
                    )}
                  </div>
